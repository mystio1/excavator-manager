...
export function AddServiceDialog({
  ...
  const { error, pending, run } = useApiForm(async (body: Record<string, unknown>) => {
    await apiFetch(`/api/excavators/${excavatorId}/service-records`, { method: "POST", body: JSON.stringify(body) });
  });
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const ok = await run({
      ...
      hourMeterAtService: Number(fd.get("hourMeterAtService")),
      items,
    });
    if (ok) {
      await mutate(`/api/excavators/${excavatorId}/service-tab`);
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="flex flex-col gap-4">
          ...
          <DialogFooter className="-mx-0 -mb-0 rounded-none border-0 bg-transparent p-0 sm:justify-stretch">
            ...
            >
              {pending ? "Saving..." : `Save Service (${selectedCount} item${selectedCount === 1 ? "" : "s"})`}
            </Button>
          </DialogFooter>
        </div>
      </DialogContent>
...
function AddComponentInline({
  ...
  const { error, pending, run } = useApiForm(async (body: { name: string; category: string }) => {
    const { component } = await apiFetch<{ component: { id: string; name: string; category: string } }>(
      "/api/service-items",
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    const fd = new FormData(e.currentTarget);
    await run({ name: fd.get("name") as string, category: fd.get("category") as string });
  }
