...
export async function ServiceTab({
  businessId,
  excavatorId,
  currentHourMeter,
  serviceStatus,
  ...
  const [catalogGroups, previousSummary, history, replacements] = await Promise.all([
    ...
    getPreviousServiceSummary(businessId, excavatorId),
    listServiceHistory(businessId, excavatorId),
    getReplacementHistory(businessId, excavatorId),
  ]);
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
                  {group.components.map((c) => {
                    ...
                    return (
                      <Link
                        key={c.id}
                        href={`/excavators/${excavatorId}/components/${c.id}`}
